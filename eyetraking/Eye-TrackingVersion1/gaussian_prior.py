...
import tensorflow as tf       #Agregado
...
class LearningPrior(Layer):
    ...
    def build(self, input_shape):
        ...
        self.trainable_weights2 = [self.W]  #New version
    ...
    def call(self, x, mask=None):
        ...
        self.b_s = x.shape[0]            #x.shape=(None, 16, 30, 40)
        ...
        x_t = np.dot(np.ones((self.height, 1)),np.linspace(0, 1.0, self.width).reshape(1,self.width)) #Nueva version para tensorflow
        x_t = tf.convert_to_tensor(x_t, dtype="float32")    #Nueva version para tensorflow
        
        #y_t = T.dot(self._linspace(e1, e2, self.height).dimshuffle(0, 'x'), T.ones((1, self.width)))
        lin = np.linspace(e1, e2, self.height)              #Nueva version para tensorflow
        y_t = np.dot(lin.reshape(lin.shape[0],1), np.ones((1, self.width)))        #Nueva version para tensorflow
        y_t = tf.convert_to_tensor(y_t, dtype="float32")    #Nueva version para tensorflow
        ...
        x_t = K.repeat_elements(K.expand_dims(x_t, axis=-1), self.nb_gaussian, axis=-1) #New version
        y_t = K.repeat_elements(K.expand_dims(y_t, axis=-1), self.nb_gaussian, axis=-1) #New version

        #print("Checkpoint 1 **************************")
        
        #print(-((x_t - mu_x) ** 2 / (2 * sigma_x ** 2 + K.epsilon())))
        ...
        gaussian = 1 / (2 * np.pi * sigma_x * sigma_y + K.epsilon()) * \
                   K.exp(-((x_t - mu_x) ** 2 / (2 * sigma_x ** 2 + K.epsilon()) +
                           (y_t - mu_y) ** 2 / (2 * sigma_y ** 2 + K.epsilon())))     #Nueva version para tensorflow
        ...
        #Corrector de self.b_s.
        if self.b_s is None:	       #AGREGADO					
            self.b_s = 1
    
        #output = K.repeat_elements(K.expand_dims(gaussian, dim=0), self.b_s, axis=0)
        output = K.repeat_elements(K.expand_dims(gaussian, axis=0), self.b_s, axis=0)  #New version
        
        print("Seccion gaussian prior ejecutada")
        return output               #TensorShape([1, 16, 30, 40])
